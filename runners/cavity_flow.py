...
sdfg = dace.SDFG.from_file("/home/chris/SPCL/npbench/runners/sdfgs/cavity_flow_auto_opt_cpu.sdfg")

class ModelSize(Enum):
    S = (61, 61, 25, 5, 1.0, 0.1)
    M = (121, 121, 50, 10, 1.0, 0.1)
    L = (201, 201, 100, 20, 1.0, 0.1)
    PAPER = (101, 101, 700, 50, 1.0, 0.1)

    def __init__(self, ny, nx, nt, nit, rho, nu):
        self.ny = ny
        self.nx = nx
        self.nt = nt
        self.nit = nit
    ...
    def init(self):
        ...
        return self.ny, self.nx, self.nt, self.nit, self.rho, self.nu, u, v, p, dx, dy, dt

def run_model(size: ModelSize):
    """Run the model with the specified size configuration."""
    ny, nx, nt, nit, rho, nu, u, v, p, dx, dy, dt = size.init()
    ...
    dy_ref = np.copy(dy)
    p_ref = np.copy(p)
    sdfg(ny=ny, nx=nx, nt=nt, nit=nit, rho=rho, nu=nu, u=u_ref, v=v_ref, dt=dt_ref, dx=dx_ref, dy=dy_ref, p=p_ref)
    ...
    dy2 = np.copy(dy)
    p2 = np.copy(p)
    sdfg_copy(ny=ny, nx=nx, nt=nt, nit=nit, rho=rho, nu=nu, _u=u2, _v=v2, _dt=dt2, _dx=dx2, _dy=dy2, _p=p2)
...
def run_all_models():
    ...
    for model_size in ModelSize:
        print(f"Running {model_size.name} model (NY={model_size.ny}, NX={model_size.nx}, NT={model_size.nt}, NIT={model_size.nit}, RHO={model_size.rho}, NU={model_size.nu})")
